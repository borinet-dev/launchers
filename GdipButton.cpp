...
BOOL CGdipButton::LoadStdImage(IStream *stream)
{
	m_pStdImage = new CGdiPlusBitmapResource;
	return m_pStdImage->Load(stream);
}
...
BOOL CGdipButton::LoadAltImage(IStream *stream)
{
	m_bHaveAltImage = TRUE;
	m_pAltImage = new CGdiPlusBitmapResource;
	return (m_pAltImage->Load(stream));
}
...
void CGdipButton::PaintBtn(CDC *pDC)
{
	...
	pDC->StretchBlt(0, 0, rect.Width(), rect.Height(), m_pCurBtn, 0, 0, m_pStdImage->m_pBitmap->GetWidth(), m_pStdImage->m_pBitmap->GetHeight(), SRCCOPY);
	*/
	CRect rect;
	GetClientRect(&rect);
	float width = (float)m_pStdImage->m_pBitmap->GetWidth();
...
LRESULT CGdipButton::OnSetCursor(WPARAM nHitTest, LPARAM message)
{
